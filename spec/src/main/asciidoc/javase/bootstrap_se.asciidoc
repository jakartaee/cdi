...
When performing lookup of bean instance with `SeContainer` the qualifiers specified in the `Instance.select()` method are used to perform typesafe resolution.
