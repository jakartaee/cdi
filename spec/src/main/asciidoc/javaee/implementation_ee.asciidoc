...
* Session beans
* Resources (Jakarta EE resources, persistence contexts, persistence units, remote Jakarta Eneterprise Beans and web services)

Jakarta EE and embeddable Jakarta Eneterprise Bean containers are required by the Jakarta EE and Jakarta Eneterprise Bean specifications to support Jakarta Eneterprise Bean session beans and the Jakarta EE component environment.
...
* It is not annotated with an Jakarta Eneterprise Bean component-defining annotation or declared as an Jakarta Eneterprise Bean bean class in `ejb-jar.xml`.



[[session_beans]]

=== Jakarta Eneterprise Bean Session beans

A _session bean_ is a bean that is implemented by a session bean with an Jakarta Eneterprise Bean 3.x client view that is not annotated with `@Vetoed` or in a package annotated `@Vetoed`. The basic lifecycle and semantics of Jakarta Eneterprise Bean session beans are defined by the Jakarta Eneterprise Bean specification.
...
==== Jakarta Eneterprise Bean remove methods of session beans

If a session bean is a stateful session bean:

* If the scope is `@Dependent`, the application _may_ call any Jakarta Eneterprise Bean remove method of a contextual instance of the session bean.
* Otherwise, the application _may not_ directly call any Jakarta Eneterprise Bean remove method of any contextual instance of the session bean.

The session bean is not required to have an Jakarta Eneterprise Bean remove method in order for the container to destroy it.

If the application directly calls an Jakarta Eneterprise Bean remove method of a contextual instance of a session bean that is a stateful session bean and declares any scope other than `@Dependent`, an `UnsupportedOperationException` is thrown.

If the application directly calls an Jakarta Eneterprise Bean remove method of a contextual instance of a session bean that is a stateful session bean and has scope `@Dependent` then no parameters are passed to the method by the container.
...
A session bean does not require any special annotations apart from the component-defining annotation (or XML declaration) required by the Jakarta Eneterprise Bean specification.
The following Jakarta Eneterprise Beans are beans:
...
=== Producer methods on Jakarta Eneterprise Bean session bean

A producer method defined in an Jakarta Eneterprise Bean session bean follows the rules defined in <<producer_method>> with the following addition:

* A producer method defined in an Jakarta Eneterprise Bean session bean must be either a business method exposed by a local business interface of the Jakarta Eneterprise Bean or a static method of the bean class.

[[declaring_producer_method_ee]]

==== Declaring a producer method in an Jakarta Eneterprise Bean session bean

A producer method declaration in an Jakarta Eneterprise Bean session bean follows the rules defined in <<declaring_producer_method>> with the following addition:
...
=== Producer field on Jakarta Eneterprise Bean session bean

A producer field defined in an Jakarta Eneterprise Bean session bean follows the rules defined in <<producer_field>> with the following addition:

* A producer field defined in an Jakarta Eneterprise Bean session bean must be a static field of the bean class.

[[declaring_producer_field_ee]]

==== Declaring a producer field in an Jakarta Eneterprise Bean session bean

A producer field declaration in an Jakarta Eneterprise Bean session bean follows the rules defined in <<declaring_producer_field>> with the following addition:

* If a non-static field of an Jakarta Eneterprise Bean session bean class is annotated `@Produces`, the container automatically detects the problem and treats it as a definition error.

[[disposer_method_ee]]

=== Disposer methods on Jakarta Eneterprise Bean session bean

A disposer method defined in an Jakarta Eneterprise Bean session bean follows the rules defined in <<disposer_method>> with the following addition:

* A disposer method defined in an Jakarta Eneterprise Bean session bean must be either a business method exposed by a local business interface of the Jakarta Eneterprise Bean or a static method of the bean class.

[[declaring_disposer_method_ee]]

==== Declaring a disposer method on an Jakarta Eneterprise Bean session bean

A disposer method declaration in an Jakarta Eneterprise Bean session bean follows the rules defined in <<declaring_disposer_method>> with the following addition:

* If a non-static method of an Jakarta Eneterprise Bean session bean class has a parameter annotated `@Disposes`, and the method is not a business method exposed by a local business interface of the session bean, the container automatically detects the problem and treats it as a definition error.

[[javaee_components]]

=== Jakarta EE components

Most Jakarta EE components support injection and interception, as defined in the Java Platform, Enterprise Edition Specification 7, table EE.5-1, but are not considered beans (as defined by this specification). Jakarta Eneterprise Beans, as defined in <<session_beans>> are the exception.
...
A _resource_ is a bean that represents a reference to a resource, persistence context, persistence unit, remote Jakarta Eneterprise Bean or web service in the Jakarta EE component environment.
...
* For a remote Jakarta Eneterprise Bean, `@Jakarta Eneterprise Bean` must be specified.
* For a web service, `@WebServiceRef` must be specified.


The injection annotation specifies the metadata needed to obtain the resource, entity manager, entity manager factory, remote Jakarta Eneterprise Bean instance or web service reference from the component environment.
...
[source, java]
----
@Produces @Jakarta Eneterprise Bean(ejbLink="../their.jar#PaymentService")
PaymentService paymentService;
----
...
A Jakarta EE or embeddable Jakarta Eneterprise Bean container must provide the following built-in beans, all of which have qualifier `@Default`:
...
* An initializer method defined in an Jakarta Eneterprise Bean session bean is _not_ required to be a business method of the session bean.

[[new_ee]]

=== `@New` qualified beans in Jakarta EE

When running in Jakarta EE, the container must extend the rules defined for managed beans in <<new>> to Jakarta Eneterprise Bean session beans.
