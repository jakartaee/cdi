...
* non-contextual instances of Jakarta Enterprise Bean session beans (for example, session beans obtained by the application from JNDI or injected using `@Jakarta Eneterprise Bean`), and
