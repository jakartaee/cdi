...
Furthermore, a bean may or may not be an alternative or a standby.
...
Alternatives take precedence over other, non-alternative beans.
...
A _standby_ is a bean that must be explicitly selected if it should be available for lookup, injection or name resolution.

Other, non-standby beans take precedence over standbys.

[[declaring_standby]]

==== Declaring a standby

A standby may be declared by annotating the bean class or producer method or field with the `@Standby` annotation.
...
Alternatively, a standby may be declared by annotating a bean, producer method or producer field with a stereotype that declares a `@Standby` annotation.

If a standby is also an alternative, the container automatically detects the problem and treats it as a definition error.

If an interceptor is a standby, non-portable behavior results.
...
* all beans with the stereotype are standbys.
...
===== Declaring a `@Standby` stereotype

A stereotype may declare a `@Standby` annotation, which specifies that every bean with the stereotype is a standby.

We may specify that all default impl objects are standbys:
...
If a stereotype declares both `@Alternative` and `@Standby`, the container automatically detects the problem and treats it as a definition error.
