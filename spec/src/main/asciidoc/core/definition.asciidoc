...
This annotation marks the bean as _automatically closed_.

If the class of a contextual instance of an automatically closed bean implements the `java.lang.AutoCloseable` interface, the `close()` method is called on the instance during destruction.
...
===== Declaring an `@AutoClose` stereotype

A stereotype may declare an `@AutoClose` annotation, which specifies that every bean with the stereotype is automatically closed.

We may specify that all beans annotated with `@CloseOnDestroy` are automatically closed:
