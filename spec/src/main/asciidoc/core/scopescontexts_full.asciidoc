...
A portable extension may define a custom context object for built-in scopes that are not always active and for custom scopes.
For example, a remoting framework might provide a request context object for the built-in request scope.
If an attempt to register a custom context object for an always active built-in scope is made, deployment problem occurs.
