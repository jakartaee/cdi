...
For the purpose of method invokers, a target method is _asynchronous_ if it matches an _async handler_.
In case of an asynchronous method, the action started by the method does not necessarily finish when the method returns; the _completion_ of the action is asynchronous to the method call.
...
If the `asyncHandler` passed to `withAsync()` is not `null`, the invoker becomes asynchronous.
The class passed to `withAsync()` must name an existing async handler that matches the target method of the invoker, otherwise deployment problem occurs.

An asynchronous invoker calls the `transform()` method of the given async handler exactly once during `invoke()`.
If the async handler is annotated `@AsyncHandler.ReturnType`, the `transform()` method is called after the target method is called, with the return value of the target method as the `original`, and the result is returned to the caller.
If the async handler is annotated `@AsyncHandler.ParameterType`, the `transform()` method is called before the target method is called, with the given argument value of the async parameter as the `original`, and the result is passed to the target method as the argument value of the async parameter.
...
It is permitted (but not recommended) to pass `null` to `withAsync()`, in which case the built invoker is not asynchronous and the requirement to destroy instances of `@Dependent` looked up beans does not change.

The CDI container must include async handlers for the following return types:

- `java.util.concurrent.CompletionStage` (used by passing `AsyncHandler.CompletionStage.class`)
- `java.util.concurrent.CompletableFuture` (used by passing `AsyncHandler.CompletableFuture.class`)
- `java.util.concurrent.Flow.Publisher` (used by passing `AsyncHandler.FlowPublisher.class`)
