...
public interface InvokerValidation {
    void ensureAsyncHandlerExists(Class<?> asyncType);
    void ensureAsyncHandlerExists(Class<?> asyncType, Supplier<String> message);
}
----

Calling `InvokerValidation.ensureAsyncHandlerExists()` makes sure that an async handler for the given async type exists.
If it does not exist, it registers a deployment problem as if `Messages.error()` was called.
