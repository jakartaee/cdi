...
* `ignoreFinalMethods()` `ignoreFinalMethods()` Instructs the container to ignore all non-static, final methods with public, protected or default visibility declared by any class in the type hierarchy of the intercepted instance during invocation of `createInterceptedInstance()` method.
...
* `createInterceptedInstance()` returns an enhanced version of the instance whose method invocations are treated as business method invocations as defined in <<biz_method>>.
