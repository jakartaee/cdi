...
===== `AnnotatedTypeConfigurator` interface

CDI 2.0 introduced the `javax.enterprise.inject.spi.builder.AnnotatedTypeConfigurator` interface to help configuring a new `AnnotatedType` instance.

`AnnotatedTypeConfigurator` must be initialized from a `java.lang.Class` or an existing `AnnotatedType` with one of its `read()` methods.

With `AnnotatedTypeConfigurator` you can perform the following operation:

* Add or remove annotation at type level with `addToType` and `removeFromType` methods.
* Add or remove annotation on a field with `addToField` and `removeFromField` methods.
* Add or remove annotation on a method with `addToMethod` and `removeFromMethod` methods.
* Add or remove annotation on a method parameter with `addToMethodParameter` and `removeFromMethodParameter` methods.
* Add or remove annotation on a constructor with `addToConstructor` and `removeFromConstructor` methods.
* Add or remove annotation on a constructor parameter with `addToConstructorParameter` and `removeFromConstructorParameter` methods.
* Remove an annotation from the type or any type member with `removeFromAll` method.
* Override a field type with `overrideFieldType` methods.
* Override a method parameter type with `overrideMethodParameterType` method.
* Override a constructor parameter type with `overrideConstructorParameterType` method.
* Change the class that the `AnnotatedType` will represent.
...
CDI 2.0 introduced the `javax.enterprise.inject.spi.builder.BeanConfigurator` interface to help configuring a new `Bean` instance.
...
CDI 2.0 introduced the `javax.enterprise.inject.spi.builder.ObserverMethodConfigurator` interface to help configuring a new `ObserverMethod` instance.
...
The method always return the same `InjectionPointConfigurator`.
...
CDI 2.0 introduced the `javax.enterprise.inject.spi.builder.InjectionPointConfigurator` interface to help configure a new `InjectionPoint` instance.
...
The method always return the same `BeanAttributesConfigurator`.
* `addDefinitionError()` registers a definition error with the container, causing the container to abort deployment after bean discovery is complete.
...
CDI 2.0 introduced the `javax.enterprise.inject.spi.builder.BeanAttributesConfigurator` interface to help configuring a new `BeanAttributes` instance.
...
With `BeanAttributesConfigurator` you can perform the following operation (defined in `GenericBeanAttributesConfigurator` interface that it extends):
...
Builders interfaces are helpers to create instances for the following SPI elements:

* `AnnotatedType` built with `javax.enterprise.inject.spi.builder.BeanAttributesBuilder`,
* `InjectionPoint` built with `javax.enterprise.inject.spi.builder.InjectionPointBuilder`,
* `BeanAttributes` built with `javax.enterprise.inject.spi.builder.BeanAttributesBuilder`,
* `Bean` built with `javax.enterprise.inject.spi.builder.BeanBuilder`
* `ObserverMethod` built with `javax.enterprise.inject.spi.builder.ObserverMethodBuilder`.

Each builder uses a configurator accessible with `configure()` method:

* `AnnotatedTypeBuilder` uses `AnnotatedTypeConfigurator` as defined in <<annotated_type_configurator>>
* `InjectionPointBuilder` uses `InjectionPointConfigurator` as defined in <<injection_point_configurator>>
* `BeanAttributesBuilder` uses `BeanAttributesConfigurator` as defined in <<bean_attributes_configurator>>
* `BeanBuilder` uses `BeanConfigurator` as defined in <<bean_configurator>>
* `ObserverMethodBuilder` uses `ObserverMethodConfigurator` as defined in <<observer_method_configurator>>

The container must provide implementation for all these builders and make them available through `ServiceLoader` for `javax.enterprise.inject.spi.builder.BuildersProvider` interface.

The class `javax.enterprise.inject.spi.builder.Builders` provides static method to obtain each builder:

[source, java]
----
public final class Builders {
    public static <T> AnnotatedTypeBuilder<T> annotatedType(Class<T> type) {}
    public static <T> AnnotatedTypeBuilder<T> annotatedType(AnnotatedType<T> type) {}
