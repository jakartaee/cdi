...
The method `BeanManager.getInstance()` returns an `Instance<Object>` to request bean instances programmatically as described in <<dynamic_lookup>>.
...
Instances of dependent scoped beans obtained with this `Instance` object should be explicitly released by calling `Instance.destroy()` method.
