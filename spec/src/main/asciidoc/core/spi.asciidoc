...
The interface `jakarta.enterprise.inject.spi.BeanManager` extends `jakarta.enterprise.inject.spi.BeanManagerLite`.
In {cdi_full} environment, `BeanManagerLite` is subject to the same rules as `BeanManager`.
...
`CDI.getBeanManager()` and `CDI.getBeanManagerLite()` may be called at any time after the container fires the `BeforeBeanDiscovery` container lifecycle event until the container fires the `BeforeShutdown` container lifecycle event.
