...
When performing lookup of bean instance with `CDI` the qualifiers specified in the `Instance.select()` method are used to perform typesafe resolution.
If no qualifier is specified in `Instance.select()` method the `@Default` qualifier is assumed.
...
When performing lookup of bean instance with the returned `Instance`, the qualifiers specified in the `Instance.select()` method are used to perform typesafe resolution.
If no qualifier is specified with `Instance.select()` method the `@Default` qualifier is assumed.
...
* Add an injection point for the producer with `addInjectionPoint` method.
* Add multiple injection points for the producer with `addInjectionPoints` methods.
* Replace all injection points for the producer with `injectionPoints` method.
