...
* `javax.enterprise.inject.spi.builder.AnnotatedTypeConfigurator` to configure an `AnnotatedType`
* `javax.enterprise.inject.spi.builder.AnnotatedFieldConfigurator` to configure an `AnnotatedField`
* `javax.enterprise.inject.spi.builder.AnnotatedConstructorConfigurator` to configure an `AnnotatedConstructor`
* `javax.enterprise.inject.spi.builder.AnnotatedMethodConfigurator` to configure an `AnnotatedMethod`
* `javax.enterprise.inject.spi.builder.AnnotatedParameterConfigurator` to configure an `AnnotatedParameter`
...
* `filterMethods()` returns a `Stream<AnnotatedFieldConfigurator>` filtered by applying the provided Predicate on `fields()`
...
`AnnotatedMethodConfigurator` and `AnnotatedConstructorConfigurator` are accessed through `AnnotatedTypeConfigurator`:
...
Both interfaces give access to `AnnotatedParameterConfigurator`:
[source, java]
----
...
`AnnotatedFieldConfigurator` is accessed through `AnnotatedTypeConfigurator`:
...
The container must provide an instance of each of these interfaces.
...
In addition `AnnotatedTypeConfigurator` is used by `AnnotatedTypeBuilder` as defined in <<builders>>
