...
If type parameter `T` is not a class non-portable behavior results.
...
The interface `javax.enterprise.inject.spi.AnnotatedConstructor` exposes the `Constructor` object.
...
If an injection point of type `InterceptionFactory` has a type parameter that is not a class, non-portable behavior results.
