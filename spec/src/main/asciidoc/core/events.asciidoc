...
`FireAsyncException` contains all exceptions thrown by observers as suppressed exceptions. They can be accessed as an array of `throwable` with the `getSuppressed` method.
