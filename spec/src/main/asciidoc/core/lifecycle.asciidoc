...
Then, if the managed bean is automatically closed and the class of the contextual instance implements `AutoCloseable`, the `close()` method is called on the instance.
...
Then, if the producer method bean is automatically closed and the class of the contextual instance implements `AutoCloseable`, the `close()` method is called on the instance.
Finally, the container destroys dependent objects, as defined in <<dependent_objects_destruction>>.
...
Then, if the producer field bean is automatically closed and the class of the contextual instance implements `AutoCloseable`, the `close()` method is called on the instance.
