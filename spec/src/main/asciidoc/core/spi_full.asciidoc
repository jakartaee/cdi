...
* `isAutoClose()` must return `true` if the bean is automatically closed, and `false` otherwise.
...
* `dispose()` only does one thing: if the bean is automatically closed and the class of the contextual instance of the bean implements `AutoCloseable`, the `close()` method is called on the instance.
...
Then, if the bean is automatically closed and the class of the contextual instance of the bean implements `AutoCloseable`, the `close()` method is called on the instance.
