...
* `isStandby()` must return `true` if the bean is a standby, and `false` otherwise.
...
* a `BeanAttributes`, which determines the bean types, qualifiers, scope, name and stereotypes of the returned `Bean`, the return values of `isAlternative()` and `isStandby()`, and
...
* `getStandbys()` returns the ordered list of enabled standbys for the application, sorted by priority in ascending order.
...
If an alternative, standby, interceptor or decorator is added using `AfterTypeDiscovery.addAnnotatedType()`, non-portable behavior results.

Any observer of this event is permitted to add classes to, or remove classes from, the list of alternatives, list of standbys, list of interceptors or list of decorators.
The container must use the final values of these collections, after all observers of `AfterTypeDiscovery` have been called, to determine the order of the enabled alternatives, standbys, interceptors, and decorators for application.
...
* Set the priority of the bean, if it is an alternative or a standby, with `priority` method.
...
* Make the bean a standby with `standby` methods.
