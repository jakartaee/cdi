...
* `isAutoClose()` must return `true` if the bean is auto-closeable, and `false` otherwise.
...
For a `Producer` that represents a managed bean:
...
* a `BeanAttributes`, which determines the bean types, qualifiers, scope, name and stereotypes of the returned `Bean`, the return values of `isAlternative()`, `isReserve()`, `isEager()`, and `isAutoClose()`, and
...
* `getReserves()` returns the ordered list of enabled reserves for the application, sorted by priority in ascending order.
...
If an alternative, reserve, interceptor or decorator is added using `AfterTypeDiscovery.addAnnotatedType()`, non-portable behavior results.

Any observer of this event is permitted to add classes to, or remove classes from, the list of alternatives, list of reserves, list of interceptors or list of decorators.
The container must use the final values of these collections, after all observers of `AfterTypeDiscovery` have been called, to determine the order of the enabled alternatives, reserves, interceptors, and decorators for application.
...
* Set the priority of the bean, if it is an alternative or a reserve, with `priority` method.
...
public interface AfterDeploymentValidation {
    ...
    public void ensureAsyncHandlerExists(Class<?> asyncType);
...
* `ensureAsyncHandlerExists()` makes sure that an async handler for the given async type exists.
If it does not exist, it registers a deployment problem as if `addDeploymentProblem()` was called.
See <<invoker_asynchronous_methods>> for more information about async handlers.
...
* Set the bean name with `name` method.
* Make the bean an alternative with `alternative` method.
* Make the bean a reserve with `reserve` method.
* Make the bean eagerly initialized with `eager` method.
* Make the bean auto-closeable with `autoClose` method.
...
The `createInvoker` method allows creating an `InvokerBuilder` for the processed bean and the given target method (see <<invoker_builder_full>>).
