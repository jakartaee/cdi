...
* A managed bean is passivation capable if and only if the bean class is serializable and all interceptors of the bean are passivation capable.
...
* has an interceptor that is not passivation capable
* has an interceptor with an injection point that is not passivation capable
...
If a managed bean which declares a passivating scope type, has an interceptor which is not a passivation capable dependency, the container automatically detects the problem and treats it as a deployment problem.
