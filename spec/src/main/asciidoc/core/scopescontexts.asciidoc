...
The `activate()` method starts the given context.  It will throw an `IllegalStateException` if the scope represented by this manageable context is already active within the current thread.
...
public class MyRunnable implements Runnable {
    public void run() {
        CDI cdi = CDI.current();
        try (ManageableContext ctx = cdi.getBeanManager().getManageableContext(RequestScoped.class).activate()) {
