...
* an instance of `InjectionPoint` representing the injection point into which the dependent object will be injected, as defined in <<injection_point>>, or
* a `null` value when no injection point exists.
...
An event with qualifier `@Initialized(RequestScoped.class)` is synchronously fired immediately after the request context is initialized.
An event with qualifier `@BeforeDestroyed(RequestScoped.class)` is synchronously fired immediately before the request context is destroyed.
An event with qualifier `@Destroyed(RequestScoped.class)` is synchronously fired immediately after the request context is destroyed.
...
An event with qualifier `@BeforeDestroyed(ApplicationScoped.class)` is synchronously fired immediately before the application context is destroyed.
An event with qualifier `@Destroyed(ApplicationScoped.class)` is synchronously fired immediately after the application context is destroyed.
...
* an event with qualifier `@Initialized(X.class)` immediately after a custom context is initialized,
* an event with qualifier `@BeforeDestroyed(X.class)` immediately before a custom context is destroyed,
* an event with qualifier `@Destroyed(X.class)` immediately after a custom context is destroyed,
