...
A bean archive might be marked as to be 'trimmed' by adding the following tag to beans.xml:
[source, xml]
<beans xmlns="http://xmlns.jcp.org/xml/ns/javaee">
    <trim/>
</beans>

If a bean archive is marked with the `<trim/>` element in it's beans.xml, only AnnotatedTypes will get processed as Beans if they

* have an explicit Scope annotation, or
* are an Interceptor or a Decorator

after the `ProcessAnnotatedType` event got fired.
`AnnotatedTypes` with no scope will not get a <<default_scope>>.
