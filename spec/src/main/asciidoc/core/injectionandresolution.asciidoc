...
A reserve may be given a priority for the application:
...
.. The container determines the highest priority value and eliminates all beans except for reserves with the highest priority value.
.. If there is exactly one bean remaining, the container selects this bean, and the ambiguous dependency is called resolvable.
