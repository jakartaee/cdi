...
The method `release()` instructs the container to destroy the instance.
The bean instance passed to `release()` should be a dependent scoped bean instance obtained from the same `Instance` object.
All other instances, including client proxies, are ignored.
Applications are encouraged to always call `release()` when they no longer require a dependent scoped bean instance obtained from `Instance`.
