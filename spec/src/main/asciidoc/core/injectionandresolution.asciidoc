...
Qualifiers specified at `Instance` injection point or via via the `Instance.select()` method are used to perform typesafe resolution.
If no qualifier is specified at `Instance` injection point or via the `Instance.select()` method the `@Default` qualifier is assumed.
