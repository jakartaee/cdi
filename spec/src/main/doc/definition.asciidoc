...
A bean class may have a _bean defining annotation_, allowing it to be placed anywhere in an application and being implicitly considered as a bean, as defined in <<bean_archive>>.

The set of bean defining annotations is :

* all normal scopes annotation (built-in or defined by third parties),
* any stereotype annotation,
* or the `@Dependent` scope annotation when explicitly added to the bean class.
...
Note that to assure compatibility with other JSR-330 implementations, all pseudo-scopes annotations except `@Dependent` *are not* bean defining annotations. However a stereotype annotation including a pseudo-scope annotation *is* a bean defining annotation.
