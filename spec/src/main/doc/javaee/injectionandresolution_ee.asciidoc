...
When running in Java EE, the container must extend the accessibility rules for beans and alternatives defined in <<selection>> to JSP/JSF pages using EL resolution.
...
When running in Java EE, the container must extend the rules defined for managed beans in <<dependent_objects_destruction>> to any other Java EE component class supporting injection.
