...
When running in Java EE, A top-level Java class is a managed bean if it meets requirement describe in <<what_classes_are_beans>> and if it is defined to be a managed bean by any other Java EE specification and if
