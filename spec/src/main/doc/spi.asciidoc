...
The container instantiates a single instance of each extension at the beginning of the application initialization process and maintains a reference to it until the application shuts down. The container delivers event notifications to this instance by calling its observer methods. The notification order for observer methods within extensions is undefined, the +@Priority+ annotation cannot be used to control the notification order.
