...
* if the class is a managed bean, session bean, or other component class supporting injection, fire an event of type +ProcessInjectionPoint+ for each injection point in the class, as defined in <<process_injection_point>>, and then
* if the class is a managed bean, session bean, or other component class supporting injection, fire an event of type +ProcessInjectionTarget+, as defined in <<process_injection_target>>, and then
