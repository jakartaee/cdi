...
Many instances of beans with scope +@Dependent+ belong to some other bean or component class instance and are called _dependent objects_.

* Instances of decorators and interceptors are dependent objects of the bean instance they decorate.
* An instance of a bean with scope +@Dependent+ injected into a field, bean constructor or initializer method is a dependent object of the bean or component class instance into which it was injected.
...
* all dependent objects of a non-contextual instance of a bean or other component class are destroyed when the instance is destroyed by the container,
