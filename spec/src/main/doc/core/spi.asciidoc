...
* +getBeanClass()+ returns the bean class of the bean or of the bean that declares the producer method or field.
...
An +InterceptionType+ identifies the kind of lifecycle callback, EJB timeout method (when used in Java EE) or business method.
...
Components may obtain an instance of +BeanManager+ from JNDI by looking up the name +java:comp/BeanManager+.
...
The container must fire an event for every injection point of every component class supporting injection that may be instantiated by the container at runtime, including every managed bean declared using +@ManagedBean+, bean, interceptor or decorator.

The event object must be of type +javax.enterprise.inject.spi.ProcessInjectionPoint<T, X>+ where +T+ is the bean class, or component class supporting injection, and +X+ is the declared type of the injection point.
...
The container must fire an event for every component class supporting injection that may be instantiated by the container at runtime, including every managed bean declared using +@ManagedBean+, bean, interceptor or decorator.

The event object must be of type +javax.enterprise.inject.spi.ProcessInjectionTarget<X>+, where +X+ is the bean class or component class supporting injection.
...
* +getAnnotatedType()+ returns the +AnnotatedType+ representing the bean class or other component class supporting injection.
...
Any observer of this event is permitted to wrap and/or replace the +InjectionTarget+. The container must use the final value of this property, after all observers have been called, whenever it performs injection upon the bean or other component class supporting injection.
