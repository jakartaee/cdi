...
* +preDestroy()+ calls the +@PreDestroy+ callback.
...
For example, this observer decorates the +InjectionTarget+ for all Maps.

[source, java]
----
<T extends Map> void decorateMap(@Observes ProcessInjectionTarget<T> pit) {
