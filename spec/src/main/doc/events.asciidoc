...
By default, the order in which observer methods are called is not defined. The +@Priority+ annotation can be applied to observer methods to specify a priority. If no +@Priority+ annotation is specified, the default priority +APPLICATION+ is assumed. Observers with smaller priority values are called first.
