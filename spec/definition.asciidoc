...
* The container must provide an implementation of the @RequestScoped, @ApplicationScoped and @SessionScoped annotations defined in <<builtin_contexts>> representing the standard scopes defined by the Java Servlets specification. As stated in <<context>> these standard scopes can be extended by third party extension by binding them to a custom `Context` object.
