...
However their corresponding scope lifecycle is a super set of Java Servlets standard scopes lifecycle (i.e. they are not limited to them). Any specification or third party development is allowed to activate them when needed so they don't require Java Servlets to be used.
