...
* The +@RequestScoped+, +@ApplicationScoped+ and +@SessionScoped+ annotations defined in <<builtin_contexts>> are inspired by the standard scopes defined by the Java Servlets specification.
However their corresponding scope lifecycle is a super set of Java Servlets standard scopes lifecycle (i.e. they are not limited to them). Any specification or third party portable extension is allowed to activate them when needed so they don't require Java Servlets to be used.
