...
Any observer of this event is permitted to wrap and/or replace the +BeanAttributes+. The container must use the final value of this property, after all observers have been called, to manage instances of the bean. Changes to +BeanAttributes+ are _not_ propagated to the annotation from which the bean definiton was created.

Any bean which has it's bean attributes altered must have it's definition validated during deployment validation.
