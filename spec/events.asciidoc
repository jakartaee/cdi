...
If an injection point which is not a parameter of an observer method with type +EventMetadata+ and qualifier +@Default+ exists, the container automatically detects the problem and treats it as a definition error.
