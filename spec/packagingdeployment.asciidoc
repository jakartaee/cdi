...
A bean archive has a _bean discovery mode_ of +all+, +annotated+ or +none+. A bean archive with no version has a bean discovery mode of +all+. A bean archive with version 1.1 (or later) has a bean discovery mode of +annotated+.
