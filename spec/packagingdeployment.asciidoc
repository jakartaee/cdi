...
An _implicit bean archive_ is any other archive which contains one or more beans with a bean defining annotation, or one or more session beans.
...
Then, container must create an +AnnotatedType+ representing the type and fire an event of type +ProcessAnnotatedType+, as defined in <<pat>>, for every type discovered.
