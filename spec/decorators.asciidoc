...
The priority of the decorator declaration, defined in <<priorities>>, determines the default decorator ordering for the application. The order of the decorator declarations for a bean archive overrides the default decorator order. If the set of decorators enabled for the application and the set of decorators enabled for a bean archive are disjoint, then the decorators enabled for the bean archive are assigned a priority by the container, such that the first decorator has a priority of 1000, the second a priority of 1010, the third a priority of 1020 and so on. Decorators which occur earlier in the list are called first.
