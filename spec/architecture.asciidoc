...
The method +fire()+ of the built-in bean of type +Event+ defined in <<eventinterface>> allows the application to fire events. Events consist of an _event object_ - in this case the +User+ - and event qualifiers. Event qualifier - such as +@LoggedIn+ and +@LoggedOut+ - allow event consumers to specify which events of a certain type they are interested in.
