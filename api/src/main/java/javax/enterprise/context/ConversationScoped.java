 ...
 * 
 * <ul>
 * <li>during all standard lifecycle phases of any JSF faces or non-faces request.</li>
 * </ul>
 * 
 * <p>
 * The conversation context provides access to state associated with a particular conversation. Every JSF request has an
 ...
 * <li>The conversation associated with a JSF request is determined at the beginning of the restore view phase and does not
 * change during the request.</li>
 * </ul>
 * 
 * <p>
 ...
 * 
 * <p>
 * If the conversation associated with the current JSF request is in the transient state at the end of a JSF request, it is
 * destroyed, and the conversation context is also destroyed.
 * </p>
 * 
 * <p>
 * If the conversation associated with the current JSF request is in the long-running state at the end of a JSF request, it is
 * not destroyed. Instead, it may be propagated to other requests according to the following rules:
 * </p>
 * 
 * <ul>
 ...
 * <li>The long-running conversation associated with a request may be propagated to any non-faces request via use of a GET
 * request parameter named <tt>cid</tt> containing the unique identifier of the conversation. In this case, the application must
 * manage this request parameter.</li>
 * </ul>
 * 
 * <p>
 * When no conversation is propagated to a JSF request, the request is associated with a new transient conversation. All
 ...
 * <li>The container is permitted to arbitrarily destroy any long-running conversation that is associated with no current JSF
 * request, in order to conserve resources.</li>
 * </ul>
 * 
